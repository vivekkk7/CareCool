...
router.get('/', (req, res) => {
    try {
        ...
        req.con.query(sq12, function (err, result, fields) {
            if (err) throw err;
...
router.post('/', mymulter.none(), async (req, res) => {
    try {
        ...
        function appointfun(userid, signed, msg) {
            ...
            var fdata = [userid, trackid, signed, apt.aemail, apt.aname, apt.amob, apt.aservice, apt.adistrict, apt.acity, moment().format('YYYY-MM-DD'), moment().format('HH:mm:ss')];
            console.log("appointfun : " + fdata);
            ...
            req.con.query(sq2, ['on', apt.adistrict], function (err, result, fields) {
                if (err) {
                    res.json({ isappoint: false, aerror: 'Unable to execute appoint query. Please try again.' });
                    return;
                }
                ...
                if (result.length > 0) {
                    ...
                    req.con.query(sql3, [fdata], function (err, result) {
                        if (err) {
                            res.json({ isappoint: false, aerror: 'Unable to execute appoint query. Try again.' });
                            return;
                        }
                        ...
                        atpmailtomech.aptMailToMech(fdata, mechemail)
                            .then(() => {
                                try {
                                    ...
                                    req.con.query(sql8, [userid], function (err, result) {
                                        if (err) {
                                        ...
                                        if (result.length == 0) {
                                            ...
                                            req.con.query(sql5, [fdata1], function (err, result) {
                                                if (err) {
                                    ...
                                    req.con.query(sql6, [fdata2], function (err, result) {
                                        if (err) {
